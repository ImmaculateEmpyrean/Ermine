...
Ermine::SubscriptionTicket Ermine::RecieverComponent::Bind(std::function<void(Event*)> Callable, std::atomic<bool>& SwitchTOControlIfAnEventCanBeExecuted, EventType SubscriptionType)
{
	...
	{
		std::unique_ptr<Ermine::OnTickEventSubscription> Obj = std::make_unique < Ermine::OnTickEventSubscription>(Callable, SwitchTOControlIfAnEventCanBeExecuted);
		return std::move(station->QueueSubscription(std::move(Obj)));
	}
	else
	{
